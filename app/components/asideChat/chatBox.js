...
export default function ChatBox({ onClose, name, targetId }) {
    ...
    const handleSubmit = async (e) => {
        ...
        console.log(`📤 [ChatBox] Sending message to ${targetId}: ${message.trim()}`)
        setIsLoading(true)
    ...
    return (
        <div className="fixed bottom-0 right-0 lg:bottom-16 lg:right-72 w-full md:w-80 h-96 bg-white rounded-t-lg shadow-lg flex flex-col z-50 border border-gray-200">
            {/* Chat Header */}
                    ...
                    conversationMessages.map((msg) => {
                        ...
                        return (
                            <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                                <div
                                    className={`max-w-[80%] p-3 rounded-lg ${isMine ? "bg-blue-500 text-white rounded-br-sm" : "bg-white text-gray-800 border rounded-bl-sm"
                                        }`}
                                >
