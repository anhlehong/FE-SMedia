...
export function InputText({ onChangeEvent, label, name, isValid, value = "" }) {
  return (
    <input
      ...
      value={value}
      className={`p-3 rounded-md outline-none w-full border-gray-300 border ${
...
export function InputDate({ onChangeEvent, label, name, isValid, value = "" }) {
  return (
    <div className="w-full">
      <h2 className="text-gray-700 mb-1 text-sm">{label}</h2>
      <input
        type="date"
        onChange={onChangeEvent}
        value={value}
...
export function InputRadioButton({
  ...
  return (
    <label
      className={`w-1/2 bg-white p-3 rounded-md outline-none relative border-gray-300 border ${
        !isValid ? "border-red-500 border" : ""
      }`}
    >
      {label}
  ...
  return (
    <div className="w-full">
      ...
      <input
        ...
        onFocus={onFocus}
        value={value}
        className={`p-3 rounded-md outline-none w-full text-center tracking-wider font-mono text-lg border-gray-300 border ${
          !isValid ? "border-red-500 border" : ""
        }`}
        name={name || "otp"}
        autoFocus
